...
class SuggestionsService(
    ...
    fun generateSuggestionStream(
        ...
        PlatformLog.d(TAG, "Generating suggestion stream: run=$runId, session=$sessionId")
        ...
        try {
            ...
            PlatformLog.d(TAG, "Suggestion stream completed")
        } catch (e: Exception) {
            PlatformLog.e(TAG, "Error generating suggestion stream", e)
            throw e
        }
    }
    ...
    suspend fun generateAssociations(
        ...
        PlatformLog.d(TAG, "Generating associations: run=$runId, session=$sessionId, count=$optionCount")
        ...
        try {
            ...
            PlatformLog.d(TAG, "Generated ${parsed.size} associations")
            return parsed
        } catch (e: Exception) {
            PlatformLog.e(TAG, "Error generating associations", e)
            throw e
        }
    }
        ...
        try {
            ...
            return options.mapNotNull { element ->
                try {
                    ...
                    PlatformLog.w(TAG, "Failed to parse option: $element", e)
                    null
                }
            }.filter { it.label.isNotBlank() && it.direction.isNotBlank() }
        } catch (e: Exception) {
            PlatformLog.e(TAG, "Failed to parse associations JSON", e)
            return emptyList()
        }
    }
}
