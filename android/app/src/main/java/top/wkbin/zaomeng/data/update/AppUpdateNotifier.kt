...
class AppUpdateNotifier(private val context: Context) {
    ...
    fun showProgress(update: AppUpdateInfo, downloadedBytes: Long, totalBytes: Long) {
        ...
        val detail = if (totalBytes > 0L) "$percentage%  ${formatAppUpdateBytes(downloadedBytes)} / ${formatAppUpdateBytes(totalBytes)}" else "已下载 ${formatAppUpdateBytes(downloadedBytes)}"
    ...
    private fun mainPendingIntent(): PendingIntent = PendingIntent.getActivity(context, NOTIFICATION_ID, Intent(context, MainActivity::class.java), PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
