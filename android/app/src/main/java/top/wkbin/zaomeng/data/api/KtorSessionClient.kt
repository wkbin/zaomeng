...
class KtorSessionClient(
    ...
    private suspend fun decodeSession(response: HttpResponse): DialogueSessionDto {
        ...
        return runCatching { json.decodeFromString<DialogueSessionDto>(text) }
            .getOrElse { error ->
                Log.e(TAG, "Failed to decode DialogueSessionDto. Response body: $text", error)
                throw error
            }
    }
    ...
    private fun url(endpoint: BackendManager.BackendEndpoint, path: String) = endpoint.baseUrl.trimEnd('/') + path
