...
class KtorPersonaClient(
    ...
    suspend fun getAvatar(runId: String, character: String): ByteArray? {
        val response = http.client.get("${url(backend.requireKtorEndpoint(), runId, character)}/avatar")
    ...
    private fun url(endpoint: BackendManager.BackendEndpoint, runId: String, character: String): String =
