...
class DialogueStreamService(
    ...
    suspend fun replyDialogueTurnStream(
        ...
        Log.d(TAG, "Starting streaming dialogue reply: run=$runId, session=$sessionId")
        ...
        try {
            ...
            try {
                ...
                Log.d(TAG, "Streaming reply retry (attempt 2/2) returned: ${retryFull.take(200)}")
            ...
            Log.d(TAG, "Streaming dialogue reply completed and committed")
        } catch (e: Exception) {
            Log.e(TAG, "Error during streaming dialogue reply", e)
            throw e
        }
    }
}
