...
class LlmClient(
    ...
    private fun loadModelSettings(): JsonObject? {
        return try {
            ...
            if (!settingsFile.exists()) {
                Log.w(TAG, "Model settings file not found: ${settingsFile.absolutePath}")
                return null
            }
            json.decodeFromString<JsonObject>(settingsFile.readText())
        } catch (e: Exception) {
            Log.e(TAG, "Failed to load model settings", e)
            null
        }
    }
    ...
    private fun getActiveProfile(): Map<String, String> {
        return try {
            ...
            Log.e(TAG, "Failed to parse model settings", e)
            emptyMap()
        }
    }
        ...
        return retryWithBackoff(maxRetries = DEFAULT_MAX_RETRIES) {
            ...
            when (response.status) {
                ...
                in RETRYABLE_STATUS_CODES.map { HttpStatusCode.fromValue(it) } -> {
                    ...
                    Log.w(TAG, "Retryable error (${response.status}): $errorBody")
    ...
    fun chatCompletionStream(
        ...
        while (true) {
            ...
            try {
                ...
                val deltaContent = delta?.content
                if (deltaContent != null && deltaContent.isNotBlank()) {
                    Log.d(TAG, "emit@${System.currentTimeMillis()} ${deltaContent.take(20)}")
                    emit(deltaContent)
                }
                delta?.reasoningContent?.takeIf { it.isNotBlank() }?.let { onReasoning?.invoke(it) }
            } catch (e: Exception) {
    ...
    private suspend fun <T> retryWithBackoff(
        ...
        repeat(maxRetries) { attempt ->
            try {
                ...
                Log.w(TAG, "Retry attempt ${attempt + 1}/$maxRetries after ${currentDelay}ms", e)
                delay(currentDelay)
