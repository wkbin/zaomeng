...
class PluginService(
    ...
    private val root get() = File(storage.getStorageRoot(), "plugins")
    private val stateFile get() = File(root, "enabled.json")
    private val builtinRoot get() = File(root, ".builtin")
    ...
    fun list(): JsonObject {
        ...
        val items = buildJsonArray {
            ...
            // 第三方插件（现有逻辑）
            root.listFiles()?.filter { it.isDirectory && !it.name.startsWith(".") }?.sortedBy { it.name }?.forEach { dir ->
    ...
    private fun itemFor(pluginId: String, enabled: Set<String>): JsonObject? {
        ...
        val manifest = File(root, "$pluginId/plugin.json")
    ...
    fun getConfig(pluginId: String): JsonObject {
        ...
        return if (config.exists()) json.decodeFromString(JsonObject.serializer(), config.readText()) else JsonObject(emptyMap())
    }

    fun updateConfig(pluginId: String, config: JsonObject): JsonObject {
        val directory = pluginDirectory(pluginId)
        directory.mkdirs()
        storage.writeTextAtomically(File(directory, "config.json"), json.encodeToString(JsonObject.serializer(), config))
        return config
    }

    /** 读取插件日志（jsonl，最多返回最近 200 条）。 */
    fun logs(pluginId: String): JsonObject {
        val logFile = File(pluginDirectory(pluginId), "plugin-logs.jsonl")
        val entries = buildJsonArray {
            if (logFile.isFile) {
                logFile.readLines().takeLast(200).forEach { line ->
    ...
    fun appendLog(pluginId: String, level: String, message: String) {
        val directory = pluginDirectory(pluginId)
        directory.mkdirs()
        val logFile = File(directory, "plugin-logs.jsonl")
        val entry = buildJsonObject {
            put("ts", Instant.now().toString())
            put("level", level)
            put("message", message)
        }
        logFile.appendText(compactJson.encodeToString(JsonObject.serializer(), entry) + "\n")
    }
    ...
    fun uninstall(pluginId: String): JsonObject {
        ...
        val trash = File(storage.getStorageRoot(), "plugin-trash").apply { mkdirs() }
        val target = File(trash, "$normalized-${System.currentTimeMillis()}")
        if (!directory.renameTo(target)) throw IllegalStateException("Unable to uninstall plugin")
        return buildJsonObject {
            put("status", "uninstalled")
            put("plugin_id", normalized)
            put("recoverable_path", target.absolutePath)
            put("uninstalled_at", Instant.now().toString())
        }
    }
    ...
    private fun isKnown(pluginId: String): Boolean = isBuiltin(pluginId) || File(root, "$pluginId/plugin.json").isFile

    private fun pluginDirectory(pluginId: String): File {
        val normalized = pluginId.trim()
        // 内置插件 id 为点分格式（com.zaomeng.*），跳过 STORAGE_ID_PATTERN 校验
        if (isBuiltin(normalized)) return File(builtinRoot, normalized)
        if (!PathSafety.STORAGE_ID_PATTERN.matches(normalized)) throw IllegalArgumentException("Invalid plugin id")
        val directory = File(root, normalized)
        if (!File(directory, "plugin.json").exists()) throw NoSuchElementException("Plugin not found: $normalized")
        return directory
    }

    private fun readEnabled(): Set<String> {
        if (!stateFile.isFile) return emptySet()
        return try {
            val value = json.parseToJsonElement(stateFile.readText()).jsonObject["enabled"]?.jsonArray
