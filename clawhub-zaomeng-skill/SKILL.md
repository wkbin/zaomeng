...
- 对用户只输出进度与结果，不输出内部思考、命令排障过程、工具选择、自检过程、失败重试细节或长段“我现在要做什么”的推理独白。
- 若 helper 脚本、依赖或路径出错，只需用一句外显状态说明当前阻塞点，不要把完整排查链路逐条展开给用户。
- 至少要播报：
  - 已锁定多少个待蒸馏角色
  - 当前正在蒸馏哪个角色
  - 当前进度是第几个 / 共几个
...
- 这个 post-process 负责补齐拆分人物文件和 `NAVIGATION.generated.md`
- helper command: `py -3 tools/materialize_persona_bundle.py --profile-file <character-dir/PROFILE.generated.md>`
