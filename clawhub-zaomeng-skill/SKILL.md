...
| `chat` | `py -3 -m src.cli.app chat` | 输出会话摘要、聊天结果、聊天状态 | `chat-status-out` with `status=complete, success=true` |
...
## 7.5 Chat Session Summary Contract

对于 `act` / `insert` / `observe` 聊天链路，宿主还可以要求 CLI 输出一份标准化会话摘要：

```bash
py -3 -m src.cli.app chat --novel <路径> --message "<请求>" --session-summary-out <session-summary.json>
```

这份 JSON 是宿主侧的标准聊天状态索引，避免宿主自己解析 session markdown。建议至少依赖这些字段：
...
- `focus_targets`
- `history_count`
- `self_insert`（仅 `insert` 模式）
- `latest_responses`（单轮执行后）
- `artifacts.session_file`
- `artifacts.relation_snapshot_file`

推荐宿主串联方式：

1. setup-only 请求后立即读取一次会话摘要
2. 每次单轮 `chat --message` 完成后再读取一次
3. 宿主 UI 或 agent 编排统一以这份摘要判断当前会话模式、参与角色、自我代入身份与最近一轮输出

宿主如果还需要一个更直接的动作结果与成功标记，可额外要求：
...
1. 先读 `chat-status-out` 判断本次调用是否成功
2. 再读 `chat-result-out` 获取动作结果
3. 如需持续跟踪会话状态，再读 `session-summary-out`
