...
class PackageSkillScriptTests(unittest.TestCase):
    def test_package_skill_archive_uses_versioned_filename_and_expected_entries(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("tools/build_persona_autofill_payload.py", names)
            self.assertIn("tools/build_dialogue_suggestion_payload.py", names)
            self.assertIn("tools/manage_self_card.py", names)
            self.assertIn("tools/export_relation_graph.py", names)
            self.assertIn("tools/_skill_support/novel_preparation.py", names)
            self.assertIn("tools/_skill_support/persona_review.py", names)
            self.assertIn("tools/_skill_support/dialogue_payloads.py", names)
